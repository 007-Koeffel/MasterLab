...
```{r Data Loading}
load("/Users/fabian/Desktop/Data/CRSP_monthly.RData") #used to import the monthly CRSP data set
load("/Users/fabian/Desktop/Data/CRSP_delist.RData") #used for importing the delisting data

CFNAI <- read_csv("/Users/fabian/Desktop/Data/CFNAI.CSV") %>% filter(date>"1967-12-29") %>% filter(date<"2019-01-01") %>% arrange(date) #CFNAI Index

Macro_Uncertainty <- read_xlsx("/Users/fabian/Desktop/Data/MacroUncertaintyToCirculate.XLSX")%>%
  mutate(date=as.Date(Date)) %>% filter(date>"1967-12-29",date<"2019-01-01") %>% rename(UNC="h=1") %>%
  arrange(Date) %>% select(date,UNC)

FF3 <- read_csv("/Users/fabian/Desktop/Data/FF_monthly_factors.CSV") %>% mutate(date=as.Date(as.yearmon(as.Date(paste0(date,"01"),format="%Y%m%d")+31))) %>%
  rename(Mkt.RF=`Mkt-RF`) %>% mutate(across(Mkt.RF:RF,~./100))

Momentum <- read_csv("/Users/fabian/Desktop/Data/FF_Momentum_Factor.CSV") %>% mutate(date=as.Date(as.yearmon(as.Date(paste0(date,"01"),format="%Y%m%d")+31))) %>%
  mutate(across(Mom,~./100))

Liquidity <- read_csv("//Users/fabian/Desktop/Data/Pastor_liquidity_factor.CSV") %>% mutate(date=as.Date(as.yearmon(as.Date(paste0(date,"01"),format="%Y%m%d")+31))) %>%
  select(date,LIQ) %>% mutate(LIQ=ifelse(LIQ==-99,NA,LIQ))

H_Factors <- read_csv("/Users/fabian/Desktop/Data/Hou_Xue_Zhang_monthly_factors.CSV") %>% mutate(date=as.Date(as.yearmon(as.Date(paste0(year*100+month,"01"),format="%Y%m%d")+31))) %>%
